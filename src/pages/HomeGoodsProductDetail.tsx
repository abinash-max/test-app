...
const HomeGoodsProductDetail = () => {
  ...
  useEffect(() => {
    ...
    // Otherwise, find product from master.json
    if (category && productNumber && masterData?.categories) {
      const categoryData = masterData.categories[category as keyof typeof masterData.categories];
  ...
  // Get room type based on category
  const getRoomType = (category: string): string => {
    if (category === "bed") return "bedroom";
    if (category === "sofa" || category === "armchair") return "living_room";
    return "bedroom";
  };
          ...
          <div className="bg-card rounded-2xl shadow-elevated p-6 animate-fade-in">
            ...
            <div className="mb-8">
              <CustomImageUploadZone
                label="Upload Room Image (JPG/JPEG)"
                image={uploadedImage}
                file={uploadedFile}
...
export default HomeGoodsProductDetail;
